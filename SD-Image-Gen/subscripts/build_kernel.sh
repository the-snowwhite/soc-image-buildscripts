...
CC_FOLDER_NAME=${4}
KERNEL_MIDDLE_NAME=${5}
KERNEL_BRANCH=${6}
KERNEL_VERSION=${7}
KERNEL_URL=${8}
KERNEL_LOCALVERSION=${9}
KERNEL_REV=${10}
PATCH_URL=${11}

echo "NOTE: running build_kernel.sh"
echo "NOTE: WORK_DIR= ${1}"
echo "NOTE: TOOLCHAIN_DIR = ${2}"
echo "NOTE: MAIN_SCRIPT_ROOT_DIR = ${3}"
echo "NOTE: CC_FOLDER_NAME = ${4}"
echo "NOTE: KERNEL_MIDDLE_NAME = ${5}"
echo "NOTE: KERNEL_BRANCH = ${6}"
echo "NOTE: KERNEL_VERSION = ${7}"
echo "NOTE: KERNEL_URL = ${8}"
...
PATCH_FILE=patches/"patch-${KERNEL_MIDDLE_NAME}.patch.xz"

ALT_SOC_KERNEL_PATCH_FILE=patches//${KERNEL_MIDDLE_NAME}-changeset.patch
GIT_PATCH_FILE=${MAIN_SCRIPT_ROOT_DIR}/${ALT_SOC_KERNEL_PATCH_FILE}

#-------------- all kernel ----------------------------------------------------------------#

# mksoc uio kernel driver module filder:
MK_KERNEL_DRIVER_FOLDER=${MAIN_SCRIPT_ROOT_DIR}/../../SW/MK/kernel-drivers
...
KERNEL_PARENT_DIR=${WORK_DIR}/arm-linux-${KERNEL_MIDDLE_NAME}-gnueabifh-kernel
...
cp ${MAIN_SCRIPT_ROOT_DIR}/socfpga.dtsi ${KERNEL_BUILD_DIR}/arch/arm/boot/dts

}

patch_git_kernel() {
if [ -f ${GIT_PATCH_FILE} ]; then #if file with that name exists
	echo ""
	echo "Sub_Scr-MSG: Git patch file found .. Applying -->"
	echo "Sub_Scr-MSG: ${GIT_PATCH_FILE}"
	cd ${KERNEL_BUILD_DIR}
...
make -j${NCORES} CROSS_COMPILE=${CC} ${KERNEL_CONF} NAME="Michael Brown" EMAIL="producer@holotronic.dk" ARCH=arm KBUILD_DEBARCH=armhf LOCALVERSION=-${KERNEL_LOCALVERSION} KDEB_PKGVERSION=$(make kernelversion)-${KERNEL_REV} deb-pkg 2>&1 | tee ../deb_rt-log.txt
